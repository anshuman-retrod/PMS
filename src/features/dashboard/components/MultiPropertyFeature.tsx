...
import { usePortfolioPropertiesQuery } from "@/services/mock/queries";

const fmtCr = (n: number) =>
  n >= 10000000 ? `₹${(n / 10000000).toFixed(2)} Cr` : `₹${(n / 100000).toFixed(1)} L`;

export function MultiPropertyFeature() {
  const { data: portfolioProperties = [] } = usePortfolioPropertiesQuery();

  const avgOcc = Math.round(
    portfolioProperties.reduce((a, p) => a + p.occupancy, 0) / portfolioProperties.length,
  );
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Portfolio command center across all Retrod properties."
      />
          ...
          {portfolioProperties.map((p) => (
            <Card key={p.id} className="p-5">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3 text-[12px]">
                <div>
                  <div className="label-uppercase text-[9px]">Occ</div>
                  <div className="font-semibold">{p.occupancy}%</div>
                </div>
                <div>
                  <div className="label-uppercase text-[9px]">RevPAR</div>
                  <div className="font-semibold">₹{p.revpar.toLocaleString()}</div>
                </div>
                <div>
                  <div className="label-uppercase text-[9px]">Alerts</div>
                  <div className="font-semibold">{p.alerts}</div>
                </div>
              </div>
              ...
              <Link
                to="/"
                className="mt-4 inline-flex items-center gap-1 text-[12px] font-medium text-primary hover:text-primary-pressed"
              >
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b border-border bg-surface-2/40 text-left">
                {["Property", "City", "Occ %", "Revenue", "RevPAR", "Alerts"].map((h) => (
                  <th
                    key={h}
