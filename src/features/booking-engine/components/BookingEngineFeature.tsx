...
import { useBookingPromosQuery } from "@/services/mock/queries";

export function BookingEngineFeature() {
  const { data: bookingPromos = [] } = useBookingPromosQuery();
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Direct booking widget, promotions, and conversion analytics."
        actions={
          <Button size="sm">
        ...
        {tab === "promos" ? (
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-border bg-surface-2/40 text-left">
                  {["Code", "Discount", "Validity", "Bookings", "Status", ""].map((h) => (
                    <th
                      key={h}
          ...
          <Card className="p-5 text-[13px]">
            ...
            <p className="mt-1 text-text-secondary">
              Widget embed code, branding, and payment gateway configuration.
            </p>
